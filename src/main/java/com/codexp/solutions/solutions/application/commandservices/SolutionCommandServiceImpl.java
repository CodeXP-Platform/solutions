...
public class SolutionCommandServiceImpl implements SolutionCommandService {
    ...
    public Solution handle(CreateSolutionCommand command) {
        ...
        var existing =
            solutionRepository.findByChallengeIdAndAuthorIdAndLanguage(
                command.challengeId(),
                command.requesterId(),
                command.language()
            );
    ...
    public Solution handle(UpdateSolutionCodeCommand command) {
        ...
        Solution solution = solutionRepository
            .findById(command.solutionId())
            .orElseThrow(SolutionNotFoundException::new);
        assertOwnershipUnlessAdmin(
            solution,
            command.requesterId(),
    ...
    public SubmitSolutionResult handle(SubmitSolutionCommand command) {
        ...
        if (
            submitContext == null ||
            submitContext.testCases() == null ||
            submitContext.testCases().isEmpty()
        ) {
            throw new ChallengeContextFetchException(
                "Challenge submit context did not include test cases."
            );
        }

        if (
            submitContext.entryFunctionName() == null ||
            submitContext.entryFunctionName().isBlank()
        ) {
            throw new ChallengeContextFetchException(
                "Challenge submit context did not include a valid entry function name."
            );
        }

        submitContext
            .testCases()
            .forEach(testCase -> validateTestInput(testCase.input()));

        solution.submit(
            Instant.now(),
            AttemptWindowMinutes.fromInt(attemptResetWindowMinutes)
        );
        Solution saved = solutionRepository.save(solution);

        Attempt queuedAttempt = Attempt.createQueued(
            AttemptId.generate(),
            saved.getId()
        );
        attemptRepository.save(queuedAttempt);

        var eventTestCases = new ArrayList<
            SolutionExecutionRequestedEvent.TestCaseData
        >();
        for (var testCase : submitContext.testCases()) {
            if (testCase.testId() == null || testCase.testId().isBlank()) {
                throw new ChallengeContextFetchException(
                    "Challenge submit context contains a test case without testId."
                );
            }

            if (testCase.expectedOutput() == null) {
                throw new ChallengeContextFetchException(
                    "Challenge submit context contains a test case without expectedOutput."
                );
            }
    ...
    private void validateTestInput(String input) {
        if (input == null || input.isBlank()) {
            throw new IllegalArgumentException(
                "Test case input cannot be blank."
            );
        }

        if (input.contains("\n") || input.contains("\r")) {
            throw new IllegalArgumentException(
                "Test case input must use comma as primary argument separator."
            );
        }

        if (input.contains("\\\\")) {
            throw new IllegalArgumentException(
                "Test case input cannot use escaped backslashes to segment arguments."
            );
        }
    }
    ...
    private void ensureAllowedRole(UserRole role) {
        if (
            ...
            throw new UnauthorizedActionException(
                "Only students or admins can manage solutions"
            );
        }
    }
    ...
    private void assertOwnershipUnlessAdmin(
        ...
        if (
            !UserRole.ROLE_ADMIN.equals(requesterRole) &&
            !solution.isOwnedBy(requesterId)
        ) {
            throw new SolutionOwnershipException(
                "Only the solution owner or admin can modify or submit this solution."
            );
        }
    }
}
