...
public class SolutionExecutionResultEventHandler {
    ...
    public void handleExecutionStarted(SolutionExecutionStartedEvent event) {
        ...
        if (
            event.data().executionId() == null ||
            event.data().executionId().isBlank()
        ) {
            throw new IllegalArgumentException(
                "Execution started event must include executionId"
            );
        }

        var attempt = attemptRepository
            .findFirstBySolutionIdAndStatusOrderByCreatedAtAsc(
                solutionId,
                AttemptStatus.QUEUED
            )
            .or(() ->
                attemptRepository.findFirstBySolutionIdAndStatusOrderByCreatedAtAsc(
                    solutionId,
            ...
            .orElseThrow(() ->
                new IllegalArgumentException(
                    "No queued attempt found for started execution event."
                )
            );
    ...
    @Transactional
    public void handleExecutionCompleted(
        SolutionExecutionCompletedEvent event
    ) {
        var solutionId = SolutionId.fromString(event.data().solutionId());
        var solution = solutionRepository
            .findById(solutionId)
        ...
        if (
            ...
            throw new IllegalArgumentException(
                "Execution completed event must include executionId"
            );
        }

        if (
            event.data().attemptId() == null ||
            event.data().attemptId().isBlank()
        ) {
            throw new IllegalArgumentException(
                "Execution completed event must include attemptId"
            );
        }

        var attemptId = AttemptId.fromString(event.data().attemptId());
        var attempt = attemptRepository
            .findById(attemptId)
            .orElseThrow(() ->
                new IllegalArgumentException(
                    "Attempt not found for completed execution event."
                )
            );

        if (
            AttemptStatus.PASSED.equals(attempt.getStatus()) ||
            AttemptStatus.FAILED.equals(attempt.getStatus())
        ) {
            return;
        }
