...
public class SolutionController {
    ...
    public ResponseEntity<SolutionResponse> create(
        ...
        var query =
            SolutionChallengeQueryAssembler.toGetSolutionByChallengeQuery(
                request.challengeId(),
                jwt.userId().value(),
                jwt.role(),
                request.language()
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(
            SolutionAssembler.toResponse(solution, latestAttempt)
        );
    }

    @GetMapping("/challenge/{challengeId}")
    public ResponseEntity<List<SolutionResponse>> getSolutionsByChallenge(
        @PathVariable String challengeId
    ) {
        var jwt = userContext.getPrincipal();

        var query =
            SolutionChallengeQueryAssembler.toGetSolutionsByChallengeQuery(
                challengeId,
                jwt.userId().value(),
                jwt.role()
            );
        ...
        var responses = solutions
            .stream()
            .map(solution -> {
                var latestAttempt = attemptQueryService
                    .handle(
                        new GetLatestAttemptBySolutionIdQuery(solution.getId())
                    )
                    .orElse(null);
                return SolutionAssembler.toResponse(solution, latestAttempt);
            })
            .toList();
    ...
    public ResponseEntity<SubmitSolutionResponse> submit(
        @PathVariable String id
    ) {
        var jwt = userContext.getPrincipal();
